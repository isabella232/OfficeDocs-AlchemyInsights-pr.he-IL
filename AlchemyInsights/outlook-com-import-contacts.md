---
title: ייבוא אנשי קשר ב-Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "278"
- "8000019"
ms.assetid: 921c5c7c-9a02-4682-9bd1-03ef9bb0fd6e
ms.openlocfilehash: 9c6cf2ee9677fdc3b29f1b29a4b29be71bd137a9
ms.sourcegitcommit: 1fb324fd156008e77b7e2008af4b3dc1c0d0ea3e
ms.translationtype: MT
ms.contentlocale: he-IL
ms.lasthandoff: 10/13/2020
ms.locfileid: "48452013"
---
# <a name="import-contacts"></a>ייבוא אנשי קשר

לפני ייבוא אנשי קשר ל-Outlook.com, יהיה עליך לייצא אותם מספק הדואר האלקטרוני האחר שלך כקובץ. csv לאחר מכן, בצע את ההוראות הבאות.
  
1. עבור אל [אנשים](https://outlook.live.com/people/) <img src='data:image/png;base64,' />
.
2. בחלק העליון של הדף, בחר באפשרות **נהל** \> את **ייבוא אנשי קשר**.
3. בחר **עיון**, נווט אל התיקיה שבה שמרת את אנשי הקשר המיוצאים ובחר **פתח**.

לקבלת מידע נוסף, ראה [ייבוא אנשי קשר ל-Outlook.com](https://support.office.com/article/285a3b55-8d93-4ac8-93df-43fffd13b2f1?wt.mc_id=Office_Outlook_com_Alchemy).

